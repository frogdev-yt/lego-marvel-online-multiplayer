...
int runClient(char* address, char* port)
{
    ...
    BOOL opt = TRUE;
    setsockopt(ConnectSocket, IPPROTO_TCP, TCP_NODELAY, (char*)&opt, sizeof(opt));

    printf("connected!");

    // detach read/send thread - should stop running if recv thread stops running
    std::thread sendThread(c_SendLoop, ConnectSocket, handle, mainPtr, mechPtr);
    sendThread.detach();

    // run receive/write thread in main loop - should return error if send thread stops running
    int test = c_RecvLoop(ConnectSocket, handle, altPtr, altMechPtr);
    if (test != 0)
    {
        return(test);
    }
...
int c_SendLoop(SOCKET sock, HANDLE handle, DWORD mainPtr, DWORD mechPtr) // read and send at 125 Hz
{
    int iSendResult;
...
int c_RecvLoop(SOCKET sock, HANDLE handle, DWORD altPtr, DWORD altMechPtr) // recv and write as fast as possible
{
